...
int save_game(char e_name[200],char mood[20],struct parameters s,struct problems_text *p,int n_nodes,int *arr_node){ //a function for saving game
...
int new_game(struct problems_text *p,char e_name[200],int n_node){  //function when emperor want start a new game
    ...
    int *arr_node;
    arr_node=(int *)calloc(n_nodes,sizeof(int));
    ...
    while((i.people!=0 && i.court!=0 && i.treasury!=0) && (i.average>=10)){
        ...
        if(decide==-1){
            ...
            if(save==1){
                ...
                save_game(e_name,mood,i,p,n_nodes,arr_node);
                getch();
                return(0);
            }
            else if(save==2){
